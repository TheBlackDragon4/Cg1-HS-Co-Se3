...
export class TriangleMeshGL {
    ...
    constructor(gl, simpleMeshIO) {
        ...
        // Normals LAB6 b)
        if (normals == null) {
            console.log("Normalen vorhanden");
            ...
            console.log("Normalen nicht vorhanden")
            const nb = gl.createBuffer();
            gl.bindBuffer(gl.ARRAY_BUFFER, nb);
    ...
    computeAverageNormals(triangles, positions) {
        ...
        // LAB6 d) 1
        const nVertices = positions.length / 3; // Gibt die Anzahl der Vertices welche durch 3 geteilt weden aus
        // Hier soll für jeden Vertex eine Normale entsprechend bereitstehen
        let normals = new Array[nVertices]; // Array muss genauso lange sein, wie wir die Anzahl an Vertices haben
        for (let i = 0; i < nVertices; i++) {
            normals[i] = new Vec3(0, 0, 0);
        }

        let nTriangles = triangles.length / 3; // Gibt die Anzahl der Dreiecke aus im Vertex aus
        for (let i = 0; i < nTriangles; i++) {
            
            // index * 3 + 0 , index * 3 + 1 , index * 3 + 2
            // LAB6 d) 2 a)
            let index = Vec3(triangles[i * 3 + 0], triangles[i * 3 + 1], triangles[i * 3 + 2]); // List die entsprechenden Werte an jeweiligen Positionen aus
